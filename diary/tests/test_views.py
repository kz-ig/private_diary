...
class LoggedInTestCase(TestCase):
    """
    各テストクラスで共通の事前準備処理をオーバーライドした独自TestCase
    テスト対象の機能はいずれもログインが必要になるため実行前にログイン処理を行う
    """
...
class TestDiaryCreateView(LoggedInTestCase):
    ...
    def test_create_diary_success(self):
        ...
        # 新規日記作成処理(Post)を実行。self.clientを使ってGetやPostといったリクエスト処理をシュミレーションできる
        # これを使ってログインやPost処理をシュミレーションして返り値にレスポンスオブジェクトを取得
        response = self.client.post(reverse_lazy('diary:diary_create'), params)

        # 日記リストページへのリダイレクトを検証。アサーションメソッド(assertXXXXという名称ルール)を使って
        # テスト結果と想定される値を比較検証
